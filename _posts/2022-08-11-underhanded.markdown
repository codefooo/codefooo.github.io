...
레퍼러(referrer)는 중개 수수료에 해당하고 1%를 정했습니다. 솔리디티에서는 소수점 연산이 불가능하므로 1%, 0.01을 분수로 표현합니다. 
...
`retireOldAdminFee`는 기존 비율이 적용된 관리 수수료를 먼저 인출하는 함수이고 `setNewAdminFee`는 새로운 수수료율을 적용하는 함수입니다. 
