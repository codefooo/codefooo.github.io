...
그래서 PBS(Proposer/Builder Separation)라는 방식을 도입하여 블록을 만드는 생성자(Builder)와 최종 블록에 서명하고 네트워크에 전파하는 제안자(Proposer)를 분리합니다. 마치 채굴자처럼 실제 트랜잭션을 모으는 것은 생성자의 역할이고 제안자(현재 비콘 체인 검증자들)는 그렇게 만들어진 블록들 중에 하나를 선택하는 방식입니다(생성자가 블록을 비딩하고 제안자가 선택).
