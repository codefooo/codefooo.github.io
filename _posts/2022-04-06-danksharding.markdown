...
블록체인에서 거래 데이터가 올바르게 저장되었는지 확인하는 방법 중 하나는 머클 트리를 이용하는 것입니다. 어떤 블록에 저장된 거래를 확인하려면 머클 증명과 머클 루트를 사용하여 확인할 수 있습니다. 하지만 머클트리는 데이터가 모두 존재함을 증명하는 목적으로는 적합하지 않습니다. 그래서 Danksharding에서는 머클 트리 대신 "KZG commitment"라는 암호학 이론을 적용합니다. 
...
그런데 다항식이라고 하는 것은, 예를 들어 degree 6의 다항식은 a + bx + cx^2 + ... + gx^6의 형태입니다. 다항식에 대한 commitment인 f(s)\*G는 타원 곡선의 성질을 이용합니다. 
f(s)\*G 의 값을 계산하면 (a + bs + cs^2 + ...)\*G 의 값이 되는데 어떤 난수 s(trusted setup에서 MPC로 생성)에 대해 s를 알지 못해도 G*(s^i)의 값들을 가지고 있기 때문에 f(s)\*G의 값을 계산할 수 있습니다.
