...
여기서 모순이 발생합니다. 앨리스가 정보를 알아낼 수 있는 어떤 방법을 가지고 있고, 정상적인 경우와 타임머신으로 "조작된" 차이를 인식하지 못한다면 
두 경우는 모두 <b>Verifier에게는 동일한</b> 상황입니다. 

그런데 구글이 타임머신을 사용한 경우는 배치 정보가 사실 "0"이라고 할 수 있습니다. 왜냐하면 타임머신을 사용한 이유가 구글조차
주파수 배치에 대한 답을 모르기 때문입니다. 그렇다면 배치 정보를 알아낼 수 있다고 하는 앨리스의 방법이라는 것은 처음부터 말이 안되는 가정이 됩니다(궤변이라는 느낌...😂).

### Optomistic Rollup

옵티미스틱 롤업은 (플라즈마 연구팀이 개발해서 그런지는 몰라도) 플라즈마의 여러 면에서 유사합니다. fraud proof 방식이고 또 L2에서 
발생하는 데이터를 L1에 전송한다는 점이 유사합니다. 플라즈마가 머클 루트 같은 트랜잭션 검증에 필요한 해쉬 값이였다면 OP롤업은 트랜잭션 데이터라는 점이 다릅니다.
