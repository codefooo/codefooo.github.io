...
파악할 수 있는 수단을 가지고 있다고 가정했습니다. 그렇다면 앨리스가 파악한 주파수 배치는 앨리스가 원하는 답이 될 수 있을까요?

여기서 모순이 발생합니다. 앨리스가 정보를 알아낼 수 있는 어떤 방법을 가지고 있고, 정상적인 경우와 타임머신으로 "조작된" 차이를 인식하지 못한다면 
두 경우는 모두 <b>Verifier에게는 동일한</b> 상황입니다. 

그런데 타임머신의 경우 구글이 제공하는 정보는 것은 "0"입니다. 왜냐하면 타임머신을 사용한 이유가 
주파수 배치에 대한 답을 모르기 때문입니다. 그렇다면 엘리스가 모자 두 개를 걷어내면서 배치 정보를 알아낼 수는 없다고 말할 수 있습니다(제가 이해를 제대로 한 것인지 모르겠지만 궤변이 아닌가 하는 느낌...😂).
